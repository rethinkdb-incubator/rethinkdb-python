...
class HandshakeV1_0(BaseHandshake):  # pylint: disable=invalid-name
    ...
    def __init__(self, host: str, port: int, username: bytes, password: bytes):
        ...
        self.__username: bytes = username.replace(b"=", b"=3D").replace(b",", b"=2C")
        self.__password: bytes = password

        self._random_nonce: bytes = bytes()
        self._first_client_message: bytes = bytes()
        self._server_signature: bytes = bytes()
    ...
    def __prepare_auth_request(self, response: str) -> bytes:
        ...
        if random_nonce != self._random_nonce:
    ...
    def __read_auth_response(self, response: str) -> None:
        ...
        json_response: Dict[str, str] = self.__decode_json_response(response)
        authentication = self.__get_authentication_message(json_response)
        server_signature: bytes = base64.standard_b64decode(authentication[b"v"])

        if not hmac.compare_digest(server_signature, self._server_signature):
